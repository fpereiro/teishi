...
(function () {
   ...
   var tester = function (fun, inputs) {
      ...
      dale.stop (inputs, false, function (v, k) {
         ...
         teishi.clog (funame + ':' + k, result);
         dale.clog ('');
         if (mismatch) {
            teishi.clog ('Mismatch!', 'Aborting test now');
            teishi.perf = false;
   ...
   function myFunctionOld (input) {
      if (teishi.type (input) !== 'array' && teishi.type (input) !== 'undefined') {
         return teishi.clog ('Input to myFunction must be either an array or undefined, but instead is', input);
      }

      if (teishi.type (input) === 'array') {
         if (input.length !== 3) {
            return teishi.clog ('Input to myFunction must be an array of length 3, but instead has length', input.length, 'and is', JSON.stringify (input));
         }
         return dale.stopNot (input, true, function (v) {
            if (teishi.type (v) === 'string') return true;
            return teishi.clog ('Each item of the input to myFunction must be a string, but instead is', v, 'with type', teishi.type (v));
         });
      }
   ...
   function validateTeishiRule (rule) {
      var metarule = [
         ...
         [teishi.type (rule) === 'array', [
            function () {
               return [(teishi.type (rule [0]) === 'string' || (teishi.type (rule [0]) === 'array' && rule [0].length === 2 && teishi.type (rule [0] [0]) === 'string' && teishi.type (rule [0] [1]) === 'string')), [
      ...
      if (teishi.v (metarule)) {
         if (teishi.type (rule) === 'array' && ! (teishi.type (rule [0]) === 'string' || (teishi.type (rule [0]) === 'array' && rule [0].length === 2 && teishi.type (rule [0] [0]) === 'string' && teishi.type (rule [0] [1]) === 'string'))) {
   ...
   function example28 (input) {
      return teishi.v ([
         ...
         ['input.last4', input.last4, false, teishi.test.equal],
   ...
   if (! example28 ({
      ...
      last4: teishi.last ([1], 0),
      last5: teishi.last ([1], 'foo'),
      last6: teishi.last ([1, 2, 3], 1),
      last7: teishi.last ([1, 2, 3], 2),
      last8: teishi.last ([1, 2, 3], 3)
   })) printError ('A test failed!');
   ...
   function example29 () {
      if (teishi.copy (circ) [0] !== '[Circular]') printError ('Circular reference not handled properly #1.');
      ...
      if (! teishi.eq (data, teishi.copy (data))) printError ('Object not copied properly #1');
      if (data === teishi.copy (data)) printError ('Object not copied properly #2');
      if (data [0] === teishi.copy (data [0])) printError ('Object not copied properly #3');
      data.push ([]);
      teishi.last (data) [0] = teishi.last (data);
      if (teishi.copy (data) [4] [0] !== '[Circular]') printError ('Circular reference not handled properly #2.');
      data.push ({});
      teishi.last (data).up = teishi.last (data);
      if (! teishi.eq (teishi.last (teishi.copy (data)), {up: '[Circular]'})) printError ('Circular reference not handled properly #3.');
      if (! teishi.eq (teishi.copy ({a: 'b', c: circ}), {a: 'b', c: ['[Circular]']})) printError ('Circular reference not handled properly #4.');
      if (! teishi.eq (teishi.copy (cinput), {a: {b: ['[Circular]', '[Circular]', '[Circular]', 1]}})) printError ('Circular reference not handled properly #5.');
      if (! teishi.eq ([[{a: 'a'}]], teishi.copy ([[{a: 'a'}]]))) printError ('Circular reference not handled properly #6.');
      var bcopy = teishi.copy (bagocats);
   ...
   if (teishi.type (teishi.time (d)) !== 'integer')                printError ('teishi.time error #3.');
   if (teishi.type (teishi.time ()) !== 'integer')                 printError ('teishi.time error #4.');
   if (teishi.type (teishi.time (/a/)) !== 'nan')                  printError ('teishi.time error #5.');
   ...
   teishi.v ('Check', [
      ...
      if (error) {
         ...
         if (isNode) teishi.clog ('Finished', 'All tests ran successfully in ' + teishi.perf + 'ms!');
         else        alert ('All tests passed successfully!');
      }
      else       teishi.clog ('There was an error with the apres function!');
   });
